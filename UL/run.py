...
for dataset in ['pen-based.arff']:#['Coil2.csv', 'iris.arff']:
    #%%
    ...
    for repeat in range(0, Restarts):
        ...
        for beta in [0.0, 0.1, 0.3]:
            ...
            nmi_avg[repeat, alg_count] = normalized_mutual_info_score(Y, Cluster_elem, average_method='arithmetic')
    ...
    summary = summary.assign(e_max = np.mean(E_max, axis=0), e_sum = np.mean(E_sum, axis=0), nmi = np.mean(nmi_avg, axis=0), time = np.mean(time_avg, axis=0))
